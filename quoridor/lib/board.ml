(** Specification of the structure of the game board and its characteristics; 
    ...
    *)
    let list_of_walls pos board = 
      ...
      Array.fold_left (fun acc (dx, dy) ->
        let newPos = (x + dx, y + dy) in
        if is_valid_position newPos && is_wall (get_cell_content newPos board) then
          Array.append acc [|newPos|]
        else
          acc
      ) [||] move_vectors
    ...
    *)
    let list_of_players pos board =
      ...
      Array.fold_left (fun acc (dx, dy) ->
        ...
        if is_valid_position newPos && is_player (get_cell_content newPos board) then
          Array.append acc [|newPos|]
        else
          acc
      ) [||] move_vectors
    ...
    *)
    let is_wall_between pos1 pos2 board =
      ...
      if pos1 = pos2 then
        raise (InvalidPosition "The two positions are the same");  
      ...
      if x1 = x2 then
        ...
        is_wall_position (x1, wall_y) && is_wall (get_cell_content (x1, wall_y) board)
      else if y1 = y2 then
        let wall_x = if x1 < x2 then x1 + 1 else x1 - 1 in
        is_wall_position (wall_x, y1) && is_wall (get_cell_content (wall_x, y1) board)
      else 
        false    
      ...
      *)
      Array.fold_left (fun acc (dx, dy) ->
        ...
        (* Handle cases where there's a player in the adjacent cell *)
          ...
          if is_valid_position jumpPos && not (Array.exists ((=) jumpPos) playersAround) then
            Array.append acc [|jumpPos|]
          else
            (* Check for valid moves around the obstructing player *)
            let adjacent_positions_around_newPos = 
              Array.map (fun (ddx, ddy) -> (x + 2 * dx + 2 * ddx, y + 2 * dy + 2 * ddy)) move_vectors
            in
            let valid_adjacent_positions = 
              Array.fold_left (fun acc pos -> 
                  if is_valid_position pos && 
                      not (Array.exists ((=) pos) playersAround) &&
                      not (is_wall_between newPos pos board) then 
                      Array.append acc [|pos|]
                  else 
                      acc
              ) [||] adjacent_positions_around_newPos in
      ...
      let rec dfs pos =
        ...
        if is_target_position pos then true
          ...
          begin
            ...
            Array.exists (fun next_pos -> 
              not visited.(snd next_pos).(fst next_pos) && dfs next_pos) next_moves
          end
      in
    ...
    let can_place_wall pos players_positions board =
      ...
      *)
      let wall_inserted =
        ...
        | 1, 0 -> 
          if x < board_size - 2 
              ...
              && not (is_wall (get_cell_content (x + 2, y) temp_board)) then
            begin
              temp_board.(y).(x) <- Wall;
        ...
        | 0, 1 ->
          if y < board_size - 2 
              ...
              && not (is_wall (get_cell_content (x, y + 2) temp_board)) then
            begin
              temp_board.(y).(x) <- Wall;
      ...
      if not wall_inserted then
        raise (InvalidWallPlacement "Wall placement is either invalid or overlaps with an existing wall");
    
      Array.for_all (fun player_pos -> dfs_path_exists player_pos temp_board) players_positions
    ...
    let place_wall pos players_positions board =
      ...
      *)
      let modify_board_for_wall () =
        match x mod 2, y mod 2 with
        | 1, 0 ->
            if x < board_size - 2 && not (is_wall (get_cell_content (x + 2, y) temp_board)) then begin
              temp_board.(y).(x) <- Wall;
              temp_board.(y).(x+2) <- Wall;
              true
            end else 
              false
        | 0, 1 ->
            if y < board_size - 2 && not (is_wall (get_cell_content (x, y + 2) temp_board)) then begin
              temp_board.(y).(x) <- Wall;
              temp_board.(y+2).(x) <- Wall;
              true
            end else 
      ...
      if Array.for_all (fun player_pos -> dfs_path_exists player_pos temp_board) players_positions then
        temp_board
      else
