...
let change_pos_of_player game pos =
  ...
  { game with board = newBoard; current_player = { game.current_player with position = pos } }
...
let rec place_wall_random game player =
  ...
  try 
      { game with board = place_wall wall_pos [|player.position|] game.board }
  with InvalidWallPlacement _ -> 
      place_wall_random game player

let det_move game player =
    let r = Random.int 2 in 
    if r == 0 then move game player
    else place_wall_random game player
