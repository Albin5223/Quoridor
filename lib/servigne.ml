...
let containsFinisLine path =
  ...
  let rec aux l =
    ...
    | [] -> None
    | (px, py) :: xs ->
        if List.exists (fun (x', y') -> x' = px && y' = py) finish then
          Some (px, py)
        else aux xs
  in
  aux path
...
let respect_prio (x, y) (new_x, new_y) (vxi, vyi) =
  let rec modulo k =
    if k = 4 then false
    else if x + (vxi * k) = new_x && y + (vyi * k) = new_y then true
    else modulo (k + 1)
  in
  modulo 1
...
let get_list_of_player () =
  let rec aux i j l =
    if i = 17 then aux 0 (j + 1) l
      ...
      try
        if get_color (i, j) <> (current_player ()).color then
          aux (i + 1) j (l @ [ (get_color (i, j), (i, j)) ])
        else aux (i + 1) j l
      with _ -> aux (i + 1) j l
  in
  aux 0 0 []
...
let is_only_one_direction (x, y) =
  let rec aux i c =
    match i with
    | 0 ->
        if isInside (x + 2, y) && not (is_wall (x + 2, y)) then
          aux (i + 1) (c + 1)
        else aux (i + 1) c
    | 1 ->
        if isInside (x - 2, y) && not (is_wall (x - 2, y)) then
    ...
    | 2 ->
        if isInside (x, y + 2) && not (is_wall (x, y + 2)) then
          aux (i + 1) (c + 1)
        else aux (i + 1) c
    | 3 ->
        if isInside (x, y - 2) && not (is_wall (x, y - 2)) then
...
let can_move (x, y) =
  ...
  let rec aux mv i (vxi, vyi) =
    ...
    | [] ->
        if i + 1 != 4 then
          aux lst_mv (i + 1) (List.nth (vectorsDirection ()) (i + 1))
        else
          let len = List.length lst_mv in
          let r = Random.int len in
          List.nth lst_mv r
    | (new_x, new_y) :: xs -> (
        (let rec already_visited fst lst =
           match lst with
           | [] -> ()
           | ((a, b), blocked) :: xs ->
               if a = x && b = y then
                 marked_case := fst @ [ ((!pastx, !pasty), true) ] @ xs
               else already_visited (fst @ [ ((a, b), blocked) ]) xs
         in
         already_visited [] !marked_case);
        ...
        | None ->
            ...
            if
              ...
              && not
                   (List.exists
                      (fun ((a, b), blocked) ->
                        a = new_x && b = new_y && blocked = true)
                      !marked_case)
            then (
              (let b = is_only_one_direction (x, y) in
               marked_case := !marked_case @ [ ((x, y), b) ]);
              (new_x, new_y))
            else aux xs i (vxi, vyi))
  in
  aux lst_mv 0 (List.nth (vectorsDirection ()) 0)
...
let need_to_block lst_ply =
  if (current_player ()).walls_left = 0 then (false, (-1, -1), (-1, -1))
    ...
    let rec who_focus (p_focus : position) (d_min : int) l : position * int =
      (* with 4 players *)
      match l with
      | [] -> (p_focus, d_min)
      | (_, (x1, y1)) :: xs ->
          let d =
            ...
            | E -> x1
          in
          if d < d_min then who_focus (x1, y1) d xs
          else who_focus p_focus d_min xs
    in
    let _, d = who_focus (999, 999) 200 lst_ply in
    if d / 2 <= 5 then
      (* if he needs 5 turn to finish i need to try to block him *)
...
let my_strategie (x, y) =
  (let x0, y0 = (current_player ()).start_position in
   (* begining of game *)
   if x = x0 && y = y0 then (
     cpt := 0;
     marked_case := [])
  ...
  let b, p1, p2 = need_to_block lst_ply in
  (* test if i need to block *)
  if b then Placing_wall (p1, p2)
  else (
    cpt := !cpt + 1;
    match placeWallQuoridor () with
    (* else i make my quoridor and then i try to finish *)
    | None ->
        let x', y' = can_move (x, y) in
        pastx := x;
        pasty := y;
        Moving (x', y')
    | Some (p1, p2) -> Placing_wall (p1, p2))
