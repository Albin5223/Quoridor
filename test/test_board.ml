...
module Strategy = struct
  ...
  let pos_wall_random () =
    let rec generate_random_wall_pos () =
      ...
      try
        validate_wall_placement (walls_left_current_player ()) (x1, y1) (x1 + xv, y1 + yv);
        ((x1, y1), (x1 + xv, y1 + yv))
      with 
...
let test_add_player_valid =
  Alcotest.test_case "add_player_valid" `Quick (fun () ->
    ...
    Alcotest.(check bool) "Player added correctly" true
...
let test_move_player_valid =
  Alcotest.test_case "move_player_valid" `Quick (fun () ->
    ...
    Alcotest.(check bool) "Player moved correctly" true
...
let test_place_wall_valid =
  Alcotest.test_case "place_wall_valid" `Quick (fun () ->
    ...
    Alcotest.(check bool) "Wall placed correctly" true
...
let test_validate_position_valid =
  Alcotest.test_case "validate_position_valid" `Quick (fun () ->
    try
      ...
      ()
    with
    | _ -> Alcotest.fail "Position validation failed unexpectedly")
...
run "Board Tests"
  [
    ("add_player_to_board", [test_add_player_valid; test_add_player_invalid_position; ]);
